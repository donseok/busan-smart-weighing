...
관리자는 웹 시스템에서 공지사항을 등록, 수정, 삭제할 수 있다.
...
2. 다음 항목을 입력한다:
   ...
   |-----------|:----:|------|
   | 제목 | O | 공지사항 제목 |
   | 카테고리 | O | 공지 분류 (시스템, 운영, 점검, 기타 등) |
   | 내용 | O | 공지사항 본문 (마크다운 지원) |
   | 첨부파일 | X | 관련 파일 첨부 |

3. **[저장]** 버튼을 클릭한다. 저장 후 기본적으로 **미게시** 상태이다.

#### 공지사항 게시/미게시

- 공지사항 목록에서 대상 공지의 **[게시]** 버튼을 클릭하면 사용자에게 공개된다.
- 게시된 공지를 비공개로 전환하려면 **[미게시]** 버튼을 클릭한다.

#### 공지사항 고정/해제

- 중요한 공지는 **[고정(📌)]** 버튼을 클릭하여 목록 상단에 고정할 수 있다.
- 고정된 공지를 해제하려면 **[고정 해제]** 버튼을 클릭한다.

### 7.7 FAQ 관리

관리자는 자주 묻는 질문(FAQ)을 등록하고 관리할 수 있다.
...
3. 다음 항목을 입력한다:
   ...
   | 입력 항목 | 필수 | 설명 |
   |-----------|:----:|------|
   | 카테고리 | O | 질문 분류 (로그인, 배차, 계량, 출문, 기타 등) |
   | 질문 | O | 자주 묻는 질문 내용 |
   | 답변 | O | 질문에 대한 답변 |
...
- FAQ 목록에서 대상 항목의 **[수정]** 또는 **[삭제]** 버튼을 클릭한다.
- 삭제된 FAQ는 즉시 사용자 화면에서 제거된다.

> **참고**: FAQ는 게시 여부 설정 없이 등록 즉시 사용자에게 노출된다.
